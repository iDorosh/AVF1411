...
var  userInterface = function(forecast, data){
	...
	if (forecast[i].condition === "Overcast"){
			icon.image = "weathericons/overcast.png";
		};
		if (forecast[i].condition === "Clear"){
			icon.image = "weathericons/sun.png";
		};
		
		if (forecast[i].condition === "Partly Cloudy"){
	...
	rowData.push(scroll);
		...
		for(var i=0, j=data.length; i<10; i++){
			...
			var degrees = Titanium.UI.createLabel({
				...
				right: "5%",
				color:"#0f70b8",
				font: {fontSize: "30%", fontFamily: "Helvetica", fontWeight: "regular"}
			});
			var day = data[i].FCTTIME.weekday_name;
			var day2 = day.toUpperCase();
			var test1 = Titanium.UI.createLabel({
				text: day2.substring(0,3),
				top: '53%',
				left: "5.5%",
				color:"#103855",
			...
			if (Ti.Platform.osname == "ipad"){
				...
				test.font = {fontSize: 40};
				test.top = "25%",
				test1.font = {fontSize: 30};
				test1.top = "54%";
				degrees.font = {fontSize: 60};
				lines1.height = 120;
			};
