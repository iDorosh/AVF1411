...
NSString * Dorosh_Ian_Project3_Weekly$ModuleRequireFormat = @"(function(exports){"
...
@implementation Dorosh_Ian_Project3_WeeklyObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_dorosh_ian_project3_weekly);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_dorosh_ian_project3_weekly gc];
}
...
{
	// create Dorosh_Ian_Project3_Weekly global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Dorosh_Ian_Project3_Weekly" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_dorosh_ian_project3_weekly = [[Dorosh_Ian_Project3_WeeklyObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_dorosh_ian_project3_weekly];
	
	NSString *_dorosh_ian_project3_weeklyNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _dorosh_ian_project3_weeklyNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_dorosh_ian_project3_weekly valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Dorosh_Ian_Project3_Weekly$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.dorosh_ian_project3_weekly.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.dorosh_ian_project3_weekly.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
