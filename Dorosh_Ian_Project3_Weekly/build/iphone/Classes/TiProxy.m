...
//Rare exceptions to indicate a bug in the _dorosh_ian_project3_weekly code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_dorosh_ian_project3_weekly = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_dorosh_ian_project3_weekly]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.dorosh_ian_project3_weekly.module"
